  ...
  return (
    <div className="flex justify-between text-white items-center">
      ...
      <span
        ...
        className="group flex items-center hover:cursor-pointer hover:text-black z-0 relative"
      >
        <p
          className={`pr-3 font-bold text-sm tracking-widest ${
            prop.dark ? "text-gray" : "text-grayish-blue"
          } ${
            prop.dark ? "group-hover:text-white" : "group-hover:text-black"
          } ${
            prop.dark ? "group-active:text-white" : "group-active:text-black"
          }`}
        >
          {prop.dark ? "LIGHT" : "DARK"}
        </p>
        {prop.dark ? (
          <svg
            width="20"
